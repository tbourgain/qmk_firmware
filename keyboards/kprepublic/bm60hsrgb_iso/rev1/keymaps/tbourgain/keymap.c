...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [1] = LAYOUT_60_iso_arrow(
        ...
        _______, _______, _______,                            _______,                   QK_LEAD, _______, KC_HOME, KC_PGDN, KC_END
    ),
};
...
void leader_end_user(void) {
    if (leader_sequence_two_keys(FR_G, FR_M)) {
       SEND_STRING("thomas.bourgain@gmail.com");
    } else if (leader_sequence_two_keys(FR_I, FR_M)) {
       SEND_STRING("thomas.bourgain@ing.com");
    } else if (leader_sequence_two_keys(FR_S, FR_I)) {
